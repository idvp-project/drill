...
<@pp.changeOutputFile name="/org/apache/drill/exec/expr/fn/impl/RepeatedEqualsFunctions.java" />
 ...
 */
public class RepeatedEqualsFunctions {

  private RepeatedEqualsFunctions() {
  }

  <#list repeatedTypes.types as type>
  @FunctionTemplate(name = "repeated_equals", scope = FunctionTemplate.FunctionScope.SIMPLE)
  public static class RepeatedContainsAny${type.type} implements DrillSimpleFunc {
    ...
    public void eval() {
      ...
      for (int i = input1.start; i < input1.end; i++) {
        ...
        equals = equals & collection.contains(d);
        <#elseif type.type == "RepeatedIntervalDay">
        sb.setLength(0);
        equals = equals & collection.contains(sb.append(holder.days).append(holder.milliseconds).toString());
        <#elseif type.type == "RepeatedInterval">
        sb.setLength(0);
        equals = equals & collection.contains(sb.append(holder.months).append(holder.days).append(holder.milliseconds).toString());
        <#elseif type.type == "RepeatedVarChar" || type.type == "RepeatedVarBinary">
        byte[] bytes = new byte[holder.end - holder.start];
        holder.buffer.getBytes(holder.start, bytes, 0, bytes.length);
        org.apache.drill.exec.expr.fn.impl.DrillByteArray v = new org.apache.drill.exec.expr.fn.impl.DrillByteArray(bytes);
        equals = equals & collection.contains(v);
        <#else>
        equals = equals & collection.contains(holder.value);
        </#if>
      }
