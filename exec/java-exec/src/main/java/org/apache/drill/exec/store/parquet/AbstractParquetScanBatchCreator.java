...
public abstract class AbstractParquetScanBatchCreator {
  ...
  protected static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(AbstractParquetScanBatchCreator.class);
  ...
  protected ScanBatch getBatch(ExecutorFragmentContext context, AbstractParquetRowGroupScan rowGroupScan,
    ...
    boolean emtpyParquetSupported = context.getOptions().getOption(ExecConstants.EMPTY_PARQUET_FILE_SUPPORTED_VALIDATOR);
    ...
    if (rowGroupScan.getRowGroupReadEntries().isEmpty()) {
      if (emtpyParquetSupported) {
        readers.addAll(createEmptyParquetReader(fsManager, rowGroupScan));
      } else {
        throw new ExecutionSetupException("Empty parquet files is not supported");
      }
    } else {
  ...
  protected abstract Collection<RecordReader> createEmptyParquetReader(AbstractDrillFileSystemManager fsManager,
  ...
  protected ParquetMetadata readFooter(Configuration conf, Path path, ParquetReaderConfig readerConfig) throws IOException {
