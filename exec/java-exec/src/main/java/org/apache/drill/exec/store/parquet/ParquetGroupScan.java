...
public class ParquetGroupScan extends AbstractParquetGroupScan {
  ...
  @Override
  public AbstractParquetRowGroupScan getSpecificScan(int minorFragmentId) {
    if (mappings.isEmpty()) {
      Set<Path> fileSet = ((ParquetMetadataProvider) metadataProvider).getFileSet();
      // Empty parquet file
      return new EmptyParquetRowGroupScan(getUserName(), formatPlugin, fileSet, columns, readerConfig, selectionRoot, filter,
